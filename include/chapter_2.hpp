...
// Solution to Q1
//------------------------------------------------------------------------
template <typename ListType, typename T> void removeDuplicates(ListType *list) {
...
template <typename ListType, typename ElemType>
ListType sumNumbersAsAListReverse(ListType &input_list1,
                                  ListType &input_list2) {
  ListType out_list{};
  ElemType carry{};
  ...
  for (auto it1 = input_list1.begin(), it2 = input_list2.begin();
       it1 != input_list1.end() && it2 != input_list2.end(); ++it1, ++it2) {
    ElemType value{};
    value = *it1 + *it2 + carry;
...
// Solution to Q7
//------------------------------------------------------------------------
template <typename ListType> bool polindrone(ListType &input_list) {
  if (input_list.empty()) {
    return true;
  }
